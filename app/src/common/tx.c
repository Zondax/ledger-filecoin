...
uint32_t tx_get_buffer_length() {
    if (tx_is_rawbytes()) return parser_rawbytes_hash_len();

    return buffering_get_buffer()->pos;
}

uint8_t *tx_get_buffer() {
    if (tx_is_rawbytes()) return parser_rawbytes_hash();
...
zxerr_t tx_rawbytes_update(uint8_t *buf, size_t buf_len) {
    if (parser_rawbytes_update(buf, buf_len) != parser_ok) return zxerr_unknown;
