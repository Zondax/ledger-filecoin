...
parser_error_t getERC20Token(const eth_tx_t *ethObj, char tokenSymbol[MAX_SYMBOL_LEN], uint8_t *decimals) {
    ...
    for (uint8_t i = 0; i < supportedTokensSize; i++) {
        if (memcmp(ethObj->legacy.to.ptr, supportedTokens[i].address, ADDRESS_CONTRACT_LENGTH) == 0) {
...
parser_error_t printERC20Value(const eth_tx_t *ethObj, char *outVal, uint16_t outValLen,
    ...
    uint256_t value = {0};
    const uint8_t *valuePtr = ethObj->legacy.data.ptr + 4 + 12 + ADDRESS_CONTRACT_LENGTH;
...
bool validateERC20(eth_tx_t *ethObj) {
    ...
    if (ethObj == NULL || isPayable || ethObj->legacy.to.rlpLen != ETH_ADDRESS_LEN ||
        ethObj->legacy.data.ptr == NULL || ethObj->legacy.data.rlpLen != ERC20_TRANSFER_DATA_LENGTH || memcmp(ethObj->legacy.data.ptr, ERC20_TRANSFER_PREFIX, 4) != 0) {
        return false;
    }
