...
parser_error_t readAddress(address_t *address, CborValue *value) {
  ...
  case ADDRESS_PROTOCOL_SECP256K1:
    // protocol 1
    PARSER_ASSERT_OR_ERROR(address->len - 1 ==
                               ADDRESS_PROTOCOL_SECP256K1_PAYLOAD_LEN,
                           parser_invalid_address)
    break;
  case ADDRESS_PROTOCOL_ACTOR:
    // protocol 2
    PARSER_ASSERT_OR_ERROR(address->len - 1 ==
                               ADDRESS_PROTOCOL_ACTOR_PAYLOAD_LEN,
                           parser_invalid_address)
    break;
  case ADDRESS_PROTOCOL_BLS:
    // protocol 3
    PARSER_ASSERT_OR_ERROR(address->len - 1 == ADDRESS_PROTOCOL_BLS_PAYLOAD_LEN,
                           parser_invalid_address)
    break;
  case ADDRESS_PROTOCOL_DELEGATED: {
    ...
    PARSER_ASSERT_OR_ERROR(address->len > actorIdSize + 1,
                           parser_invalid_address)
    break;
  }
  default:
...
parser_error_t parse_cid(cid_t *cid, CborValue *value) {
  ...
  if ((uint8_t)tag == TAG_CID) {
    CHECK_CBOR_MAP_ERR(
        cbor_value_copy_byte_string(value, tmp, &cid_len, NULL /* next */))
    ...
    bytes_read +=
        decompressLEB128(tmp + bytes_read, cid_len - bytes_read, &version);
    decompressLEB128(tmp + bytes_read, cid_len - bytes_read, &codec);

    if ((uint8_t)codec != CID_CODEC || (uint8_t)version != CID_VERSION ||
        (uint8_t)base != CID_BASE)
...
parser_error_t printCid(cid_t *cid, char *outVal, uint16_t outValLen,
  ...
  *outBuffer = 'b';
  ...
  if (base32_encode(cid->str, cid->len, outBuffer + 1, sizeof(outBuffer) - 1) ==
      0) {
...
parser_error_t parse_check_prefix(CborValue *value, const char *prefix,
                                  uint16_t prefix_len) {
  ...
  if (len != prefix_len)
    zemu_log_stack("different lengths");
