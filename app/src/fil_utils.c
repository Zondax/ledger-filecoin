...
parser_error_t print0xToF0(const uint8_t *ethAddress, uint8_t ethAddressLen, char *outVal, uint16_t outValLen,
...
parser_error_t printAddress(const address_t *a, char *outVal, uint16_t outValLen, uint8_t pageIdx, uint8_t *pageCount) {
    // the format :
...
parser_error_t printEthAddress(const address_t *a, char *outVal, uint16_t outValLen, uint8_t pageIdx,
                               uint8_t *pageCount) {
...
parser_error_t parser_printBigIntFixedPoint(const bigint_t *b, char *outVal, uint16_t outValLen, uint8_t pageIdx,
                                            uint8_t *pageCount, uint16_t decimals) {
...
parser_error_t renderByteString(uint8_t *in, uint16_t inLen, char *outVal, uint16_t outValLen, uint8_t pageIdx,
                                uint8_t *pageCount) {
...
parser_error_t parse_cid(cid_t *cid, CborValue *value) {
    ...
    // CID is a custom type tagged with 42, as described here:
...
parser_error_t parse_check_prefix(CborValue *value, const char *prefix, uint16_t prefix_len) {
    zemu_log_stack("parse_check_prefix");
    ...
    if (len != prefix_len) zemu_log_stack("different lengths");

    zemu_log_stack("Copied prefix:");
    zemu_log_stack(copied);

    is_equal = strncmp(copied, prefix, prefix_len);
