...
view_error_t h_addr_update_item(uint8_t idx) {
    ...
    switch (idx) {
        case 0:
            // address = [PK][length byte address][byte address (protocol + payload)][length string address][string address*]
            snprintf(viewdata.addr, MAX_CHARS_ADDR, "%s",
                     (char *) (G_io_apdu_buffer + PK_LEN + ADDRESS_PROTOCOL_SECP256K1_PAYLOAD_LEN +
                               ADDRESS_PROTOCOL_LEN + 2));
            break;
        case 1:
